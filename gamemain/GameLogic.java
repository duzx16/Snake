...
public class GameLogic {
    ...
    public static final Point[] dir_pos = {new Point(0, -1), new Point(0, 1), new Point(-1, 0), new Point(1, 0)};
    ...
    public static void killSnake(int index, GameData data) {
        _mask.addDeadSnake(index, data);
        data.snake_nums[index] -= 1;
        initSnake(index, data);
    }

    public static void initSnake(int index, GameData data) {
    ...
    private static boolean neighborSpare(GameMap map, Point pos) {
        for (int j = 0; j < 4; j++) {
            Point neighbor = pos.add(dir_pos[j]);
            neighbor.bound();
    ...
    private static Dir randomDir(GameMap map, Point pos, Point last_dir) {
        ...
        int n = 0;
        for (int i = 0; i < 4; i++) {
            if (!dir_pos[i].equalTo(last_dir)) {
                Point new_pos = pos.add(dir_pos[i]);
    ...
    static void initWalls(GameMap map, ArrayList<ArrayList<Point>> walls) {
        ...
        for (int i = 0; i < GameConstants.wall_num; i++) {
            ...
            do {
                ...
                if (next == null)
                    break;
                Point next_dir = dir_pos[next.ordinal()];
    ...
    public static void snakeStep(int index, GameData data, Dir dir) {
        Snake snake = data.snakes[index];
        if (snake.size() > 0) {
            Point head_pos = snake.body.elementAt(0).add(dir_pos[dir.ordinal()]);
            head_pos.bound();
            switch (snake.state) {
                case FREE:
                    switch (data.map.elementAt(head_pos).type) {
                        case FOOD:
                            ...
                            snake.body.setElementAt(dir_pos[dir.ordinal()].minus(), 1);
    ...
    // 检测两条蛇是否发生了互相碰撞
    public static void snakeCrash(GameData data) {
    ...
    // 蛇从洞口出来
    public static void snakeOut(int index, GameData data, Hole hole) {
